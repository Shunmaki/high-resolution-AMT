        ...
        with h5py.File(hdf5_path, 'r') as hf:
            if hf.attrs['split'].decode() == split:
                ...
                if n == 90:
                    ...
                    if 'pedal_frame_output' in output_dict.keys():
                        total_dict1['ref_pedal_on_off_pairs'] =                             np.array([[event['onset_time'], event['offset_time']] for event in pedal_events])
                    ...
                    if 'pedal_frame_output' in output_dict.keys():
                        total_dict2['ref_pedal_on_off_pairs'] =                             np.array([[event['onset_time'], event['offset_time']] for event in pedal_events])
